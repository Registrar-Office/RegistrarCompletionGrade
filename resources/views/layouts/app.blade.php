            ...
            <div class="fixed inset-y-0 left-0 z-50 bg-green-700 w-64 shadow-md hidden sm:block">
                ...
                <nav class="mt-5 px-2 overflow-y-auto h-full">
                    ...
                    @elseif(Auth::check() && Auth::user()->role === 'faculty')
                        <a href="{{ route('faculty.dashboard') }}" class="mt-1 group flex items-center px-2 py-2 text-sm font-medium rounded-md {{ request()->routeIs('faculty.dashboard') ? 'bg-green-800 text-white' : 'text-green-100 hover:bg-green-600 hover:text-white' }}">
                            <svg class="mr-3 h-6 w-6 {{ request()->routeIs('faculty.dashboard') ? 'text-green-300' : 'text-green-200 group-hover:text-green-100' }}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                            Faculty Dashboard
                        </a>
                        <a href="{{ optional(\App\Models\Course::where('instructor_name', Auth::user()->id_number)->first()) ? route('faculty.grade-checklist', ['course' => \App\Models\Course::where('instructor_name', Auth::user()->id_number)->first()->id]) : '#' }}" class="mt-1 group flex items-center px-2 py-2 text-sm font-medium rounded-md {{ (request()->routeIs('faculty.grade-checklist') || request()->routeIs('faculty.student-checklist')) ? 'bg-green-800 text-white' : 'text-green-100 hover:bg-green-600 hover:text-white' }}">
                            <svg class="mr-3 h-6 w-6 {{ (request()->routeIs('faculty.grade-checklist') || request()->routeIs('faculty.student-checklist')) ? 'text-green-300' : 'text-green-200 group-hover:text-green-100' }}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <a href="{{ route('rules.index') }}" class="mt-1 group flex items-center px-2 py-2 text-sm font-medium rounded-md {{ request()->routeIs('rules.*') ? 'bg-green-800 text-white' : 'text-green-100 hover:bg-green-600 hover:text-white' }}">
                        <svg class="mr-3 h-6 w-6 {{ request()->routeIs('rules.*') ? 'text-green-300' : 'text-green-200 group-hover:text-green-100' }}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
